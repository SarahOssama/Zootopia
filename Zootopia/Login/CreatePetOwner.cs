        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {  //el controller el mafroud yenafez order el insert hena
                    string usern = "OW-" + textBoxUserName.Text;
                    if (textBoxNewLocation.Text == "")
                    {
                        ...
                        else
                        {
                            ...
                            int r = controllerObj.InsertPetOwner(int.Parse(phonenum_textbox.Text), fname_textbox.Text,lname_textbox.Text,usern, int.Parse(comboBox1.SelectedValue.ToString()));
                    ...
                    else
                    {
                        ...
                        if (locid == null)   //not already an existant location
                        {
                            ...
                            if (r == 0)
                            {
                                MessageBox.Show("An error occured , please repeat");
                            }
                        }
                        int l = controllerObj.InsertLogin(usern, textBoxPassword.Text);
                        DataTable Lidt = controllerObj.SelectLocID(textBoxNewLocation.Text);
                        int lid = int.Parse(Lidt.Rows[0][0].ToString());
                        int r_l = controllerObj.InsertPetOwner(int.Parse(phonenum_textbox.Text), fname_textbox.Text, lname_textbox.Text, usern, lid);
                        if (r_l == 0)
                        {
                            MessageBox.Show("An error has occured , please repeat");
                        }
